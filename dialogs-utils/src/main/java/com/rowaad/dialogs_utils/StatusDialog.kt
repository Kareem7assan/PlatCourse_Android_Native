...
object StatusDialog {
    ...
    fun show(
        activity: Activity, title: String, status: Boolean, @DrawableRes successRes:Int=R.drawable.ic_ok ,seconds: Int = 2,
        ...
        val dialog = CustomDialogBuilder(activity, R.layout.status_dialog)
            ...
            ?.image(R.id.iv_icon, if (status) R.drawable.ic_ok else R.drawable.ic_false)
        ...
        iconBackground.backgroundTintList = ColorStateList.valueOf(
            ContextCompat.getColor(
                activity,
                if (status) android.R.color.holo_blue_bright else android.R.color.holo_red_light
            )
        )
        ...
        toastBackground.backgroundTintList = ColorStateList.valueOf(
            ContextCompat.getColor(
                activity,
                if (status) R.color.apple else R.color.sunflower_yellow
            )
        )

...
class ContactBottomDialog(private val onAction:(title:String)->Unit) : BottomSheetDialogFragment() {

    private var bindSheet: BottomContactBinding? = null

    override fun setupDialog(dialog: Dialog, style: Int) {
        val view = View.inflate(context, R.layout.bottom_contact, null)
        ...
        bindSheet?.addContactTeacher?.setOnClickListener {
            when {
                bindSheet?.etMsg?.text?.isBlank()==true -> {
                    toast(getString(R.string.msg_required))
                }
                else -> {

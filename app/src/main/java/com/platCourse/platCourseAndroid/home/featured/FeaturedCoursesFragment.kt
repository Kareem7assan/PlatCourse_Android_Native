package com.platCourse.platCourseAndroid.home.featured
...
class FeaturedCoursesFragment : BaseFragment(R.layout.fragment_my_courses) {
    ...
    private fun setupTitle() {
        (requireActivity() as HomeActivity).setupTitleSubCat(getString(R.string.featured_courses))
    }
    ...
    private fun handlePage() {
        binding.scrollView.handlePagination {
            pageNumber++
            if (hasNext()) sendRequestFeaturedCourses()

        }
    }

    private fun sendRequestFeaturedCourses() {
        viewModel.sendRequestFeaturedCourses(pageNumber)
    }
